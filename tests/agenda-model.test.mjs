...
import {
  ...
  startOfWeek
} from "../public/js/models/agenda-model.js";
...
test("preserva o tipo estruturado do atendimento", () => {
  const event = normalizeAgendaEvent({
    type: "appointment",
    appointmentType: "physical-assessment",
  ...
  assert.equal(event.appointmentType, "physical-assessment");
  assert.equal(event.title, "Avaliação física");

  const legacy = normalizeAgendaEvent({
    type: "appointment",
  ...
  assert.equal(legacy.appointmentType, "__custom");
});
