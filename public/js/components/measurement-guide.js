...
const guides = {
  waist: {
    ...
    purpose: "É essencial para estimar o percentual de gordura por circunferências. O cálculo combina cintura, pescoço e altura; na equação feminina, também utiliza o quadril.",
    steps: [
  ...
  neck: {
    ...
    purpose: "É essencial para estimar o percentual de gordura por circunferências, em conjunto com cintura e altura.",
    steps: [
  ...
  hip: {
    ...
    purpose: "É essencial para a estimativa feminina de percentual de gordura por circunferências. Nos demais casos, pode ser acompanhado como uma medida corporal opcional.",
    steps: [
    ...
    button.addEventListener("click", (event) => {
      ...
      dialog.querySelector("#measurement-guide-content").innerHTML = `
        <div class="guide-purpose">
          <strong>Por que esta medida é solicitada?</strong>
          <p>${escapeHtml(guide.purpose)}</p>
        </div>
        <h3>Como realizar a medição</h3>
...
export function measurementHelpButton(measurement) {
  ...
  const label = guide?.title || "Como realizar esta medição";
  return `<button class="measurement-help-button" data-measurement-help="${measurement}" type="button" aria-label="${escapeHtml(label)}" title="${escapeHtml(label)}">?</button>`;
}
