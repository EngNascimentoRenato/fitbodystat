...
function saveChangeToCloud(userId, stateToSave, change) {
  ...
  if (change?.type === "profile-plan") {
    const requests = [saveProfileAndPlan(userId, stateToSave, actor, change)];
...
function renderPatientContext() {
  ...
  const maskedEmail = (() => {
    ...
    if (!domain) return "";
    return `${name.slice(0, Math.min(2, name.length))}${"•".repeat(Math.max(3, Math.min(6, name.length - 2)))}@${domain}`;
  })();
