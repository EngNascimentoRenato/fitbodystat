...
function bindMeasurementForm(state, persist, render) {
  ...
  const updateBodyFatFields = () => {
    ...
    if (calculator) {
      const caliperEnabled = methodField?.value === "caliper";
      calculator.hidden = !caliperEnabled;
      calculator.querySelectorAll('input:not([type="hidden"])').forEach((input) => {
        input.disabled = !caliperEnabled;
      });
    }
