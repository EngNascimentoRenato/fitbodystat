...
const appointmentTypes = {
  training: [
    ...
    ["guided-training", "Treino acompanhado"],
    ["reassessment", "Reavaliação"],
    ["training-guidance", "Orientação de treino"],
    ["group-class", "Aula coletiva"],
    ["online-service", "Atendimento online"]
  ],
  nutrition: [
    ["initial-consultation", "Consulta inicial"],
    ["nutrition-consultation", "Consulta nutricional"],
    ["follow-up", "Retorno"],
...
function professionalAppointmentTypes(profile = {}) {
  const profession = profile.professionType;
  const group = ["personal-trainer", "physical-educator"].includes(profession)
    ? "training"
    : ["nutritionist", "nutrologist"].includes(profession) ? "nutrition" : "general";
  return appointmentTypes[group];
}

function appointmentTypeValue(event, options) {
  if (!event.id && !event.appointmentType && event.title === "Atendimento") {
    return options[0]?.[0] || "__custom";
  }
  if (event.appointmentType && options.some(([value]) => value === event.appointmentType)) {
    return event.appointmentType;
  }
  return options.find(([, label]) => label === event.title)?.[0] || "__custom";
}
...
function eventTitle(event) {
  ...
  return event.title || "Atendimento";
}
...
function renderDayView(days, groups, patients, availability, readOnly = false) {
  const date = days[0];
  ...
  return `
    <section class="agenda-day" aria-label="Agenda de ${escapeAttribute(formatDate(date))}">
      <header>
        ...
        ${readOnly ? "" : `<button class="icon-button agenda-add-date" type="button"
      ...
      <div class="agenda-day-list">
        ${events.map((event) => {
          ...
          `;
        }).join("") || `<div class="agenda-empty-day"><p>Nenhum compromisso neste dia.</p>${readOnly ? "" : `<button class="icon-button agenda-add-date" type="button" data-create-on-date="${escapeAttribute(date)}" aria-label="Adicionar compromisso" title="Adicionar compromisso">+</button>`}</div>`}
      </div>
    </section>
  `;
}
...
function renderEditor(patients, professionalProfile = {}) {
  ...
  const typeOptions = professionalAppointmentTypes(professionalProfile);
  const appointmentType = isBlock ? "" : appointmentTypeValue(event, typeOptions);
  ...
  return `
    <dialog class="agenda-dialog" id="agenda-dialog">
      <form class="form agenda-form" id="agenda-form" data-profession-type="${escapeAttribute(professionalProfile.professionType || "")}">
        ...
        <fieldset class="agenda-form-section">
          ...
          <div class="field agenda-appointment-title-field" ${isBlock ? "hidden" : ""}>
            <label for="agenda-appointment-type">Tipo de atendimento</label>
            <select id="agenda-appointment-type" name="appointmentType">
              ${typeOptions.map(([value, label]) => `
                <option value="${escapeAttribute(value)}" ${appointmentType === value ? "selected" : ""}>${escapeHtml(label)}</option>
              `).join("")}
              <option value="__custom" ${appointmentType === "__custom" ? "selected" : ""}>Outro</option>
            </select>
          </div>
          <div class="field agenda-custom-title-field" ${isBlock || appointmentType !== "__custom" ? "hidden" : ""}>
            <label for="agenda-custom-title">Descrição do atendimento</label>
            <input id="agenda-custom-title" name="customTitle" maxlength="80"
              value="${escapeAttribute(appointmentType === "__custom" ? event.title || "" : "")}" />
          </div>
          <div class="field agenda-block-title-field" ${!isBlock ? "hidden" : ""}>
            <label for="agenda-title">Motivo da indisponibilidade</label>
            <input id="agenda-title" name="title" maxlength="80" value="${escapeAttribute(isBlock ? event.title || "Indisponível" : "")}" />
          </div>
          <div class="field">
...
function renderEventDetails(event, patients, readOnly = false) {
  if (!event) return "";
  const patient = selectedPatient(event, patients);
  const isBlock = event.type === "block";
    ...
    <dialog class="agenda-dialog agenda-details-dialog" id="agenda-details-dialog">
      <article>
        ...
        <div class="agenda-detail-heading">
          ...
          ${readOnly || isBlock || event.status === "cancelled" ? renderStatusBadge(event.status) : `
        ...
        <dl class="agenda-detail-grid">
          ...
          ${event.status === "cancelled"
            ...
            : ""}
          ${readOnly ? "" : detailValue("Observações privadas", event.privateNotes, { wide: true })}
        </dl>

        <footer class="agenda-dialog-actions">
          ${readOnly ? "<span></span>" : !isBlock && event.status === "cancelled"
          ...
          <div class="button-row">
            ...
            ${readOnly ? "" : `<button class="button primary" id="edit-agenda-event" type="button">${isRecurring ? "Editar ocorrência ou série" : "Editar"}</button>`}
          </div>
        </footer>
      </article>
    </dialog>
...
export function renderAgenda(authState) {
  ...
  const presented = presentation
    ? anonymizeAgendaData(sourceEvents, sourcePatients)
    : { events: sourceEvents, patients: sourcePatients };
  const events = presented.events;
  const patients = presented.patients;
  ...
  const calendar = agendaUi.view === "day"
    ? renderDayView(days, groups, patients, authState.agendaAvailability, presentation)
    : agendaUi.view === "month"
  ...
  return `
    <div class="view-stack agenda-view">
      <section class="agenda-toolbar">
        ...
        <div class="agenda-toolbar-actions">
          ...
          <div class="button-row">
            ${presentation ? "" : `<button class="button primary" data-new-agenda-event="appointment" type="button">Novo compromisso</button>`}
          </div>
        </div>
    ...
    ${agendaUi.detailOpen ? renderEventDetails(detailEvent, patients, presentation) : ""}
    ...
    ${agendaUi.editorOpen && !presentation ? renderEditor(patients, authState.professionalProfile) : ""}
    ${agendaUi.availabilityOpen && !presentation ? renderAvailabilityEditor() : ""}
  `;
}
...
function updateEditorVisibility() {
  ...
  const customTitle = form.elements.appointmentType?.value === "__custom";
  ...
  document.querySelector(".agenda-appointment-title-field")?.toggleAttribute("hidden", isBlock);
  document.querySelector(".agenda-custom-title-field")?.toggleAttribute("hidden", isBlock || !customTitle);
  document.querySelector(".agenda-block-title-field")?.toggleAttribute("hidden", !isBlock);
  ...
  if (form.elements.appointmentType) form.elements.appointmentType.disabled = isBlock;
  if (form.elements.customTitle) {
    form.elements.customTitle.disabled = isBlock || !customTitle;
    form.elements.customTitle.required = !isBlock && customTitle;
  }
  if (form.elements.title) {
    form.elements.title.disabled = !isBlock;
    form.elements.title.required = isBlock;
  }
...
function eventInputFromForm(form, patients) {
  ...
  const typeOptions = professionalAppointmentTypes({
    professionType: form.dataset.professionType || ""
  });
  const appointmentType = type === "appointment" ? data.appointmentType || "__custom" : "";
  const selectedType = typeOptions.find(([value]) => value === appointmentType);
  return {
    ...data,
    type,
    appointmentType,
    title: type === "block"
      ...
      : appointmentType === "__custom" ? data.customTitle : selectedType?.[1] || data.customTitle || "Atendimento",
...
export function bindAgenda(context) {
  ...
  document.getElementById("agenda-appointment-type")?.addEventListener("change", updateEditorVisibility);
