    ...
    <section class="card">
      ...
      <form class="form" id="professional-profile-form">
        <div class="form-grid">
          ...
          <div class="field">
            <label for="account-profession-type">Profissão</label>
            <select id="account-profession-type" name="professionType" required>
              <option value="" ${profile.professionType && ["personal-trainer", "physical-educator", "nutritionist", "nutrologist"].includes(profile.professionType) ? "" : "selected"} disabled>Selecione uma profissão</option>
...
export function renderAccount(state, authState) {
  ...
  const email = protectIdentity ? "Informação ocultada" : user.email || "-";
...
export function bindAccount(context) {
  ...
  document.getElementById("professional-profile-form")?.addEventListener("submit", async (event) => {
    ...
    const professionType = data.get("professionType");
    const professionalProfile = {
      ...(context.authState.professionalProfile || {}),
      name: String(data.get("name") || "").trim(),
      professionType,
      careArea: careAreaForProfession(professionType),
