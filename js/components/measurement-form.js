export function measurementFields(profile, entry = {}) {
  return `
    <div class="field">
      ...
      <input id="waistCm" name="waistCm" inputmode="decimal" required value="${entry.waistCm ?? ""}" />
    </div>
    <div class="field">
      <label for="neckCm">Pescoço (cm)</label>
      <input id="neckCm" name="neckCm" inputmode="decimal" value="${entry.neckCm ?? profile.startNeckCm ?? ""}" />
      <span class="help-text">Necessário para estimar gordura por medidas.</span>
    </div>
    <div class="field">
      <label for="hipCm">Quadril (cm)</label>
      <input id="hipCm" name="hipCm" inputmode="decimal" value="${entry.hipCm ?? profile.startHipCm ?? ""}" />
